INPUT <- "The applicaiton expects a csv or tsv file as input."
ORIGINAL <- "In the Original Data Summary tab a sample of the first rows of the 
        uploaded raw table is displayed, along with a box that displays a summary and 
structure of the data. The following tab, Preprocessed Data displays two tables, 
Visit data and Patient data, along with buttons to download or display the summary and structure of 
each table. Patient data contains only the constant(non time series) variables, such as patient's sex.
Visit data contains all variables (time-series and constant)."

FILTER <- "The next tab is the Filter Data tab, a functionality that allows the user to choose either visit data or patient data, and to select variables to group and filter. The filter elements on the left are used to group the data, a pre-filter. This is used to create a subset of data to apply the filter on, while the rest of the data remains unchanged.
...
PANEL <- "This is a panel data creation tool that turns the visit time into a timestamp and allows the user to construct a time-series longitudinal panel according to preferences. User can either cut off time points and aggregate data for each patient at each time interval, or keep all the time points.
...
tabPanel(
    ...
    mainPanel(
        ...
        shiny::tags$div(class = "container-fluid shiny-code-container well",
                        HTML(MISSING)),
        tags$h3("Create a new patient feature"),
        shiny::tags$div(class = "container-fluid shiny-code-container well",
                        HTML(NEW_PATIENT_VAR)),
        tags$h3("Create a new categorical feature"),
        shiny::tags$div(class = "container-fluid shiny-code-container well",
                        HTML(NEW_CAT_VAR)),
        tags$h3("Outliers"),
        shiny::tags$div(class = "container-fluid shiny-code-container well",
